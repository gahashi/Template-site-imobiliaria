...
const AddFilerForm = ({ formData, setFormData }) => {
  ...
  const { getRootProps, getInputProps } = useDropzone({
    onDrop: acceptedFiles => {
      setFormData(prevData => ({
        ...prevData,
        files: [...(prevData.files || []), ...acceptedFiles]
      }))
    },
  ...
  return (
    <Box>
      <Box
        ...
        sx={{ border: `1.5px solid ${theme.palette.text.disabled} `, py: 3, borderRadius: 1 }}
      >
        <input {...getInputProps()} />
        <Box sx={{ display: 'flex', textAlign: 'center', alignItems: 'center', flexDirection: 'column' }}>
          ...
          <Typography variant='h4' sx={{ mb: 2.5 }} fontSize={isMd ? 24 : 18}>
            Click para adicionar fotos do seu imovel
          </Typography>
          <Typography sx={{ color: 'text.secondary' }} fontSize={isMd ? 16 : 10}>
            (Preferencia a png jpng)
          </Typography>
        </Box>
      </Box>
